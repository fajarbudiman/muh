...
if ($modul == 'manajemen_user' AND $act == 'input'){
	if (empty($_POST[NUPTK]) || empty($_POST[Username]) || empty($_POST[Password]) || empty($_POST[Level]) || empty($_POST[Email]) || empty($_POST[Aktif])){
	...
	elseif ($_POST[PendidikanTerakhir] == '++' OR $_POST[Agama] == '++'){
		...
		$numRowsNIP		 = mysql_num_rows(mysql_query("SELECT NIP FROM tuser WHERE NIP = '$_POST[NUPTK]'"));
		if ($numRowsUsername > 0){
			echo "<script language='javascript'>alert('Username sudah digunakan, gunakan username lain.');
					window.location = '../../master.php?module=manajemen_user&act=tambahuser'</script>";
		}
		elseif ($numRowsNIP > 0){
			echo "<script language='javascript'>alert('NUPTK sudah digunakan, gunakan NUPTK lain.');
					window.location = '../../master.php?module=manajemen_user&act=tambahuser'</script>";
		}
		else{
			$createdDate = date('Y-m-d H:i:s');
			$passwordEnkrip = md5($_POST[Password]);
			mysql_query("INSERT INTO tuser( NUPTK,
											NamaLengkap,
											Alamat,
											Telepon,
											CellPhone,
			...
			echo "<script language='javascript'>alert('User $_POST[NamaLengkap] dengan NUPTK = $_POST[NUPTK] berhasil ditambahkan / disimpan');
